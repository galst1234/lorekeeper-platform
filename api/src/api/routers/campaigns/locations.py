...
class LocationResponse(CampaignEntityResponse):
...
class CreateLocationRequest(CampaignEntityCreateModel):
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "slug": "moonlit-tavern",
...
class PatchLocationRequest(CampaignEntityPatchModel):
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
...
async def patch_location(
    ...
    location = await location_service.get_location_by_slug(
        db, member.campaign_id, location_slug, member.role, for_update=True
    )
    if location is None:
        raise HTTPException(status_code=404, detail="Location not found")
