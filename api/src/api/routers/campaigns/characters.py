...
def _to_response(character: Character, storage: ImageStorage) -> CharacterResponse:
    return CharacterResponse(
        ...
        image_url=storage.url_for(character.image_key) if character.image_key else None,
...
async def list_characters(
    ...
    return [_to_response(character, storage) for character in characters]
...
async def create_character(
    ...
    return _to_response(character, storage)
...
async def upload_character_image(
    ...
    try:
        updated = await character_service.set_character_image(db, character, new_key, storage)
    except Exception:
...
async def delete_character_image(
    ...
    if character is None:
        raise HTTPException(status_code=404, detail="Character not found")
    await character_service.clear_character_image(db, character, storage)
